...
def combine_data(database, suffix):
    ...
    logger.debug(f"Found {len(species_list)} species in {database}.")
    ...
    for species in tqdm(species_list):
        ...
        try:
            ...
            logger.debug(f"No file found for {species}. Error: {e}")
            continue
    ...
    assert not df.empty, f"Empty dataframe for {database}."
    logger.success(
...
def add_b73(df: pd.DataFrame) -> pd.DataFrame:
    ...
    b73_dir = (
        config.data_processed / "Maize_nam" / "zmb73"
    )  # replace "Maize_nam" with "Maize" and "zmb73" with "Zmb73" if needed
    species = "zm-b73"
...
if __name__ == "__main__":
    ...
    for database in [
        ...
        "Ensembl",
        "Refseq",
    ]:  # Add these if necessary 'Maize', 'Maize_addition',
