...
def extract_flanking_region(annot_name, annot_path, db_name):
    ...
    exe_str = (
        # NOTE this needs to be looked into depending on the OS,
        # otherwise the file will be empty;
        # -P for ubuntu, -p for linux and macOS
        f"grep -p '\tgene\t' {in_path} > {out_path}"
    )
    utils.execute(exe_str)
    assert os.path.exists(
        out_path
    ), f"Failed to extract flanking region for {annot_name} in {db_name}. File doesn't exist."
    assert os.path.getsize(out_path) > 0, (
        f"Failed to extract flanking region for {annot_name} in {db_name}. File is empty. "
        "This could be due to the wrong grep command or incorrect grep installation. "
        "Consider changing the `-p` to `-P` in `extract_flanking_region` if you're on ubuntu, "
        "or doing `brew install grep` if you're on mac."
    )
...
def subtract(dna_name, annot_name, annot_path, db_name):
    ...
    exe_str = f"{config.bedtools} subtract -a " + os.path.join(annot_path, annot_1kbup)
...
def extract_sequence(dna_name, annot_name, dna_path, annot_path, save_path, db_name):
    ...
    exe_str = f"{config.bedtools} getfasta -fi " + os.path.join(dna_path, dna_name)
...
def generate_sequence_for_species(
    dna_name,
    annot_name,
    dna_url,
    annot_url,
    ...
    relevant sequences from the initial .fa/fna dna file and .gff3/gff
...
def load_processed_fa(processed_db_path, dna_name, db_name, species_name):
    ...
    assert os.path.exists(
        f_path
    ), f"Failed to load sequences for {species_name} in {db_name}. File doesn't exist. Path: {f_path}"
    assert (
        os.path.getsize(f_path) > 0
    ), f"Failed to load sequences for {species_name} in {db_name}. File is empty. Path: {f_path}"
    fasta_sequences = SeqIO.parse(open(f_path), "fasta")
    assert (
        fasta_sequences
    ), f"Failed to load sequences for {species_name} in {db_name}. Path: {f_path}"
